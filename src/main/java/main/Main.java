	...
	public static void main(String[] args)
		...
		switch (chargeOption) {
			case "1":
				...
				int customerAge = QueryAge.query();
				...
				Identity identity = new Identity(customerAge, memberAuth, groupAuth);
			...
			case "2":
				...
				int numOfPeople = QueryNumOfPeople.queryAll();
				if (numOfPeople < 10) {
					int leaseTerm = QueryRentalTime.queryLeaseTerm();
					int priceOfSingle = CompetitionPool.priceOfRentSingleWay(leaseTerm);
				} else {
					int numOfCoach = QueryNumOfPeople.queryCoach();
					int priceOfCompetitionPool = CompetitionPool.priceOfCompetitionPool(numOfPeople, numOfCoach);
